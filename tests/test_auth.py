...
def auth_token(client, registered_user):
    response = client.post(
        "/api/v1/auth/token",
        data={"username": registered_user["email"], "password": "testpassword"},
    )
...
class TestProtectedEndpoints:
    ...
    def test_protected_endpoint_with_invalid_token_returns_401(self, client):
        response = client.get(
            "/api/v1/tenants/",
            headers={"Authorization": "Bearer totally.invalidtoken.token"},
        )
        assert response.status_code == 401

    def test_protected_endpoint_with_valid_token_returns_200(self, client, auth_token):
        response = client.get(
            "/api/v1/tenants/", headers={"Authorization": f"Bearer {auth_token}"}
        )
        assert response.status_code == 200
