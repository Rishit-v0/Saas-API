...
async def create_tenant(
    ...
    existing_tenant = db.query(models.Tenant).filter(
        models.Tenant.slug == tenant_data.slug
        ).first()
    if existing_tenant:
        raise HTTPException(
...
async def list_my_tenants(
    ...
    tenants = db.query(models.Tenant).join(
        models.TenantUser,
        models.TenantUser.tenant_id == models.Tenant.id
        ).filter(
...
async def invite_user_to_tenant(
    ...
    existing_membership = db.query(models.TenantUser).filter(
    ...
    return {"detail": f"User {invite_data.email} invited to tenant '{tenant.name}' with role '{invite_data.role}'"}
...
async def list_tenant_members(
    ...
    members = db.query(models.TenantUser).filter(
        models.TenantUser.tenant_id == tenant.id
    ).all()
    return members